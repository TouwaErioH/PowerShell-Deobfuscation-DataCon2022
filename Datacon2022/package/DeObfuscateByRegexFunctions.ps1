...
function ReplaceQuotesFunctionName
    {
       ...
       While ($matches.Count -gt 0){
            ...
            ForEach($match in $matches){
                ...
                $TempScript = $Script.Replace($match, $match.ToString().replace('"','').replace("'",""))
...
$pattern = [regex] $regex
     $matches = $pattern.Matches($Script) 
	   ...
	   $OldScript = $Script
            ForEach($match in $matches){
                $TempScript = $Script.Replace($match, $match.ToString().replace('"','').replace("'","").replace(".","").replace("&",""))
				if (GoodSyntax $TempScript ) { $Script = $TempScript  }
                }
            $matches = $pattern.Matches($Script) 
       ...
       While ($matches.Count -gt 0){
	    $OldScript = $Script
            ForEach($match in $matches){
                ...
                $TempScript = $Script.Replace($match, $match.ToString().replace('.','').replace('&','').replace("('",'').replace("')",'').replace('("','').replace('")',''))
				if (GoodSyntax $TempScript ) { $Script = $TempScript  }
                }
            $matches = $pattern.Matches($Script) 
        if ($Script -eq $OldScript) { break; }
       ...
       While ($matches.Count -gt 0){
	    $OldScript = $Script
            ForEach($match in $matches){
                ...
                $TempScript = $Script.Replace($match, $ReplacedValue )
				if (GoodSyntax $TempScript ) { $Script = $TempScript  }
                }
            $matches = $pattern.Matches($Script) 
			if ($Script -eq $OldScript) { break; }
       ...
       While ($matches.Count -gt 0){
	    $OldScript = $Script
            ForEach($match in $matches){
                ...
                $TempScript = $Script.Replace($match, "'$($SolvedString)'")
				if (GoodSyntax $TempScript ) { $Script = $TempScript  }
                }
            $matches = $pattern.Matches($Script)
			if ($Script -eq $OldScript) { break; }
...
function RemoveTicks
 ...
 $tick = Get-Content package\tick.txt
